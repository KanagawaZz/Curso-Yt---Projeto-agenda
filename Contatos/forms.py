...
class EditarContatoForm(forms.Form):
    def __init__(self, *args, **kwargs):
        ...
        for i, email in enumerate(contato.email_set.all()):
            self.fields[f"email_{i}"] = forms.EmailField(max_length=250, label=f"Email {i+1}", initial=contato.email_set.all() [i].endereco)
...
def clean_nome_contato(self):
    ...
    for cntt in Contato.objects.all():
        nomes_contatos.append(cntt.nome)
    nomes_contatos.remove(self.contato.nome)
    if nome_contato in nomes_contatos:
        raise forms.ValidationError("O nome escolhido ja esta sendo ultilizado")
    return nome_contato