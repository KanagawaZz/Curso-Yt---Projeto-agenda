...
def editar_contato(request, contato_id):
    contato = get_list_or_404(Contato, id=contato_id)
    if request.method =='POST':
        ...
        if form.is_valid():
            ...
            for nome, valor in cd.itens():
                if nome.startwith('tel_'):
                    cd_tels.append(valor)
                elif nome.startwith('email_'):
                    cd_emails.append(valor)
                elif nome.startwith('g_'):
                    if valor:
                        grupo = Grupo.objetcs.get(nome=nome.replace('g_', ''))
                        cd_grupos.append(grupo)
            contato_tels = contato.telefone_set.all()
            contato_emails = contato.emails_set.all()
            contato_grupos = contato.grupos_set.all()
